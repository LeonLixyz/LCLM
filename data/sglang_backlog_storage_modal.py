"""Storage-only successor; preparation never dispatches GPU generation."""
from __future__ import annotations
import json
from pathlib import Path
import modal
from data.sglang_backlog import MODEL, REVISION, IMAGE, SOURCES, atomic_json, file_sha, sha, read_jsonl
from data.sglang_backlog_fast import checked_rows
from data.sglang_backlog_storage import (generation_config, verify_storage_only_config,
    continuation_coverage, predecessor_path, validate_continuation_decision, validate_resume_decision,
    validate_reused_probe, validate_never_attempted, compact_completion,
    INPUT_VOLUME, OUTPUT_VOLUME, ORIGIN_MANIFEST_SHA256, ORIGIN_DECISION_SHA256, EXPECTED_REMAINING)

APP_NAME = 'lclm-sglang27-continuation-storage-20260909-v3'
ROOT = Path('/runs/stage3-build-20260906/sglang27-continuation-storage-20260909-v3')
ORIGIN = Path('/data/stage3-build-20260906/sglang27-backlog-fast-20260909-v2')
app = modal.App(APP_NAME)
inputs = modal.Volume.from_name(INPUT_VOLUME, create_if_missing=False)
volume = modal.Volume.from_name(OUTPUT_VOLUME, create_if_missing=True, version=2)
...
def origin_gate():
    from data.sglang_backlog_fast_modal import load_gate
    if file_sha(ORIGIN / 'manifest.json') != ORIGIN_MANIFEST_SHA256:
        raise ValueError('Original probe manifest changed')
    if file_sha(ORIGIN / 'probe-decision.json') != ORIGIN_DECISION_SHA256:
        raise ValueError('Original root probe decision changed')
    return load_gate(ORIGIN_DECISION_SHA256, 'probe')
...
def tests():
    ...
    result = subprocess.run(['python', '-m', 'pytest', '-q',
        'tests/test_sglang_backlog_storage.py', 'tests/test_sglang_backlog_fast.py',
        'tests/test_sglang_backlog.py', 'tests/test_sglang_throughput_smoke.py',
        'tests/test_expansion_source_probes.py', 'tests/test_expansion_retry_diagnostic.py',
        'tests/test_harvest_expansion_trace.py'], cwd='/opt/lclm', capture_output=True, text=True)
    print(result.stdout, result.stderr, flush=True)
    if result.returncode:
        raise RuntimeError('Storage successor CPU tests failed')
...
def prepare():
    inputs.reload()
    volume.reload()
    origin, original_decision, _ = origin_gate()
    config = generation_config()
    verify_storage_only_config(config, origin['generation_config'])
    test = json.loads((ROOT / 'tests.json').read_text())
    if test['status'] != 'passed' or test['generation_config'] != config:
        raise ValueError('Tests do not cover this successor snapshot')
    sources, counts, ids_sha = continuation_coverage(origin)
    if counts != EXPECTED_REMAINING or sum(s['rows'] for s in sources) != 272287:
        raise ValueError('Exact remaining272287 IDs changed')
    manifest = {'status': 'prepared_pending_storage_and_source_review', 'sources': sources,
        'pending_rows': 272287, 'pending_counts': counts, 'remaining_task_ids_sha256': ids_sha,
        'probe_rows': 896, 'original_pending_rows': 273183, 'generation_config': config,
        'origin_probe_manifest_path': str(ORIGIN / 'manifest.json'),
        'origin_probe_manifest_sha256': ORIGIN_MANIFEST_SHA256,
        'origin_probe_decision_path': str(ORIGIN / 'probe-decision.json'),
        'origin_probe_decision_sha256': ORIGIN_DECISION_SHA256,
        'original_excluded_pilot_attempts': origin['excluded_pilot_attempts'],
        'original_accepted_ids': origin['original_accepted_ids'],
        'original_accepted_ids_sha256': origin['original_accepted_ids_sha256'],
        'maud_ontology_path': origin['maud_ontology_path'],
        'maud_ontology_sha256': origin['maud_ontology_sha256'],
        'pubmed_split_sha256': origin['pubmed_split_sha256'],
        'storage': config['storage_successor'], 'approved_for_generation': False,
        'approved_for_release': False, 'selection': 'Exact original896 probes plus disjoint272287 continuation IDs; no payload rewrite.'}
    path = ROOT / 'manifest.json'
    if path.exists() and json.loads(path.read_text()) != manifest:
        raise ValueError('Refusing changed prepared storage manifest')
    atomic_json(path, manifest)
    volume.commit()
    return {'manifest_sha256': file_sha(path), 'output': str(ROOT),
            'pending_rows': 272287, 'probe_rows': 896, 'pending_counts': counts,
            'remaining_task_ids_sha256': ids_sha, 'output_volume': OUTPUT_VOLUME,
            'output_volume_version': 2, 'approved_for_release': False}


def resume_gate(manifest):
    origin, original_decision, _ = origin_gate()
    path = ROOT / 'resume-probe-decision.json'
    decision = json.loads(path.read_text())
    fresh, reused = validate_resume_decision(decision, manifest, origin, original_decision,
        manifest_sha=file_sha(ROOT / 'manifest.json'))
    for source in origin['sources']:
        name = source['source']
        directory = ORIGIN / 'outputs' / name / 'chunk-00000'
        if name in reused:
            report_path = directory / 'report.json'
            if file_sha(report_path) != reused[name]:
                raise ValueError('Reused original probe report changed')
            report = json.loads(report_path.read_text())
            validate_reused_probe(report, source=source)
            # Bind original task results and raw bytes without copying or regenerating them.
            for key, digest in report['result_hashes'].items():
                result_path = directory / 'results' / (key + '.json')
                if file_sha(result_path) != digest:
                    raise ValueError('Reused original result changed')
                result = json.loads(result_path.read_text())
                raw = directory / 'attempts' / key / f"attempt-{result['attempt']:02d}" / 'raw.json'
                if file_sha(raw) != result['raw_responses_sha256']:
                    raise ValueError('Reused original raw response changed')
        elif name in fresh:
            validate_never_attempted(directory)
    return origin, original_decision, decision, fresh, reused


def probe_path(source, reused):
    return (ORIGIN if source in reused else ROOT) / 'outputs' / source / 'chunk-00000' / 'report.json'


def load_gate(decision_sha256, stage):
    manifest = json.loads((ROOT / 'manifest.json').read_text())
    if manifest['generation_config'] != generation_config():
        raise ValueError('Successor code/configuration changed')
    origin, original_decision, resume, fresh, reused = resume_gate(manifest)
    resume_sha = file_sha(ROOT / 'resume-probe-decision.json')
    if stage == 'probe':
        if decision_sha256 != resume_sha:
            raise ValueError('Resume probe decision changed')
        return manifest, resume, fresh
    if stage != 'continuation':
        raise ValueError('Unknown generation stage')
    path = ROOT / 'continuation-decision.json'
    if file_sha(path) != decision_sha256:
        raise ValueError('Continuation decision changed')
    decision = json.loads(path.read_text())
    if decision.get('resume_probe_decision_sha256') != resume_sha:
        raise ValueError('Continuation does not bind the storage probe-routing decision')
    probes = {}
    for name in decision.get('source_reviews', {}):
        path = probe_path(name, reused)
        probes[name] = {'sha256': file_sha(path), 'report': json.loads(path.read_text())}
    allowed = validate_continuation_decision(decision, manifest, origin, original_decision, probes,
        manifest_sha=file_sha(ROOT / 'manifest.json'), origin_manifest_sha=ORIGIN_MANIFEST_SHA256,
        origin_decision_sha=ORIGIN_DECISION_SHA256)
    return manifest, decision, allowed


def reused_probe_routes():
    return json.loads((ROOT / 'resume-probe-decision.json').read_text())['reused_probe_reports']
...
def check_gate(decision_sha256: str, stage: str):
    ...
    return {'ready_for_generation': True, 'stage': stage, 'allowed_sources': sorted(allowed),
        'reused_probes': reused_probe_routes(), 'pending_rows': manifest['pending_rows'],
...
class Generator:
    ...
    def chunk(self, source_name, index, decision_sha256, stage):
        ...
        binding = {'source': source_name, 'chunk': index, 'input_sha256': spec['sha256'],
            ...
            'origin_probe_manifest_sha256': ORIGIN_MANIFEST_SHA256,
            'origin_probe_decision_sha256': ORIGIN_DECISION_SHA256,
            'resume_probe_decision_sha256': file_sha(ROOT / 'resume-probe-decision.json')}
...
def finish_report(manifest, stage, allowed, state, path):
    ...
    report['resume_probe_decision_sha256'] = file_sha(ROOT / 'resume-probe-decision.json')
...
@app.function(**cpu_options, cpu=2, memory=8192, timeout=1800)
def reconcile_original_probes():
    """Read-only check; root supplies the stopped-call assertion in its decision."""
    inputs.reload()
    origin, decision, _ = origin_gate()
    output = {'origin_probe_manifest_sha256': ORIGIN_MANIFEST_SHA256,
              'origin_probe_decision_sha256': ORIGIN_DECISION_SHA256,
              'reused_probe_reports': {}, 'never_attempted_probe_sources': [], 'requires_reconciliation': {}}
    for source in origin['sources']:
        name = source['source']
        if name not in decision['probe_sources']:
            continue
        directory = ORIGIN / 'outputs' / name / 'chunk-00000'
        path = directory / 'report.json'
        try:
            if path.exists() and json.loads(path.read_text()).get('status') == 'complete':
                validate_reused_probe(json.loads(path.read_text()), source=source)
                output['reused_probe_reports'][name] = file_sha(path)
            else:
                validate_never_attempted(directory)
                output['never_attempted_probe_sources'].append(name)
        except Exception as exc:
            output['requires_reconciliation'][name] = {'type': type(exc).__name__, 'message': str(exc)}
    return output
