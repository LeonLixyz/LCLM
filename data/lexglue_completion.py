...
from data.sglang_lexglue_defined import generation_config as generator_config,ONTOLOGY_PATH
from data.expansion_training_quality import inspect_expansion_efficiency

GENERATION_ROOT=Path('/runs/stage3-build-20260906/sglang27-lexglue-defined-20260909-v4')


def configuration():
    names=('lexglue_completion.py','lexglue_completion_modal.py','expansion_trace_audit.py',
           'audit_qwen_agent_format.py','synthetic_expansion_agent.py','real_expansion_agent.py')
    return {'version':'corrected-lexglue-private-completion-v1','cpu_only':True,
...
def governing(policy_path,policy_sha,review_spec,manifest_spec):
    ...
    core.require(review.get('reviewed_by')=='root' and review.get('decision')=='continue_corrected_lexglue'
...
def freeze_selection(policy_path,policy_sha,review_spec,manifest_spec,terminal_spec,base,destination):
    ...
    with tempfile.TemporaryDirectory(prefix='corrected-lex-selection-') as temp:
        ...
        with (output/'training-exclusions.jsonl').open('x') as handle:
            for spec in source['chunks']:
                ...
                if spec['index']==0:
                    core.require(report_spec['sha256']==review['source_reviews']['lex_glue']['probe_report_sha256'],
                        'Corrected probe changed after root review')
                else:
                    ...
                    core.require(report_spec=={'path':saved['report_path'],'sha256':saved['report_sha256']},
                        'Corrected source result report changed after terminal accounting')
    ...
    core.require(counts['results']==manifest['original_pending_rows'],'Corrected probe+continuation coverage is incomplete')
