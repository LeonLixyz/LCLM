"""Inert config-selective successor; CPU preparation requires actual root selection."""
...
from data.sglang_backlog_storage import compact_completion,ORIGIN_MANIFEST_SHA256,ORIGIN_DECISION_SHA256
from data.sglang_lexglue_selected import (ROOT,PARENT_ROOT,generation_config,validate_selection,prepare_chunk,
    finalize_subset,validate_continuation,PARENT_PROBE_DECISION_SHA256,optional_progress)
from data.lexglue_task_definition_correction import VERSION

APP_NAME='lclm-sglang27-lexglue-selected-20260909-v5'
ORIGIN=PARENT_ROOT
...
def selection(selection_sha):
    spec={'path':str(ROOT/'config-decision.json'),'sha256':selection_sha}
    decision,parent=validate_selection(spec)
    return spec,decision,parent


@app.function(**cpu_options,cpu=2,memory=8192,timeout=1800)
def tests():
    import subprocess
    result=subprocess.run(['python','-m','pytest','-q','tests/test_sglang_lexglue_selected.py',
        'tests/test_sglang_backlog_storage.py','tests/test_sglang_backlog_fast.py'],
        cwd='/opt/lclm',capture_output=True,text=True)
    print(result.stdout,result.stderr,flush=True)
    if result.returncode:raise RuntimeError('Config-selective successor CPU tests failed')
    return {'status':'passed','output':result.stdout,'generation_config':generation_config(),
        'production_subset_prepared':False,'gpu_dispatched':False}


@app.function(**cpu_options,cpu=2,memory=8192,timeout=3600,max_containers=4,retries=0)
def prepare_one(selection_sha:str,index:int):
    inputs.reload();volume.reload();spec,decision,parent=selection(selection_sha);source,=parent['sources']
    report=prepare_chunk(source['chunks'][index],ROOT/'inputs'/f'parent-{index:05d}',
        config_allowlist=decision['config_allowlist'],canary_ids=source['probe_ids'],selection_spec=spec)
    volume.commit()
    return {'index':index,'selected':report['selected_rows'],'counts':report['counts']}


@app.function(**cpu_options,cpu=4,memory=16384,timeout=7200)
def finalize_preparation(selection_sha:str):
    inputs.reload();volume.reload();spec,decision,parent=selection(selection_sha)
    reports=[{'path':str(ROOT/'inputs'/f'parent-{c["index"]:05d}'/'report.json'),
              'sha256':file_sha(ROOT/'inputs'/f'parent-{c["index"]:05d}'/'report.json')}
             for c in parent['sources'][0]['chunks']]
    manifest=finalize_subset(parent,spec,decision,reports,ROOT/'accounting')
    path=ROOT/'manifest.json'
    if path.exists() and json.loads(path.read_text())!=manifest:raise ValueError('Selected manifest changed')
    atomic_json(path,manifest);volume.commit()
    return {'manifest_sha256':file_sha(path),'manifest_path':str(path),'config_allowlist':manifest['config_allowlist'],
        'selected_rows':manifest['pending_rows'],'held_config_rows':manifest['plan_accounting']['held_config_unattempted'],
        'diagnostic_canaries':64,'approved_for_release':False}


@app.function(**cpu_options,cpu=1,memory=2048,timeout=86400,retries=0)
def prepare(selection_sha:str):
    inputs.reload();volume.reload();_,_,parent=selection(selection_sha)
    for report in prepare_one.starmap([(selection_sha,c['index']) for c in parent['sources'][0]['chunks']],order_outputs=False):
        print(json.dumps(report),flush=True)
    return finalize_preparation.remote(selection_sha)


def load_gate(decision_sha256,stage):
    if stage!='continuation':raise ValueError('All64 v4 canaries are diagnostic only; this app cannot run probes')
    manifest=json.loads((ROOT/'manifest.json').read_text())
    selection_spec,decision,parent=selection(manifest['config_selection']['sha256'])
    if manifest['config_selection']!=selection_spec or manifest['config_allowlist']!=decision['config_allowlist']:
        raise ValueError('Selected manifest differs from reviewed config choice')
    from data.sglang_lexglue_selected import document
    current=document({'path':str(ROOT/'continuation-decision.json'),'sha256':decision_sha256})
    allowed=validate_continuation(current,manifest,manifest_sha=file_sha(ROOT/'manifest.json'),selection_sha=selection_spec['sha256'])
    return manifest,current,allowed


def reused_probe_routes():return {}
def probe_path(source,reused):return PARENT_ROOT/'outputs/lex_glue/chunk-00000/report.json'
def predecessor_path(source,index,**unused):
    return probe_path(source,{}) if index==1 else ROOT/'outputs'/source/f'chunk-{index-1:05d}'/'report.json'


@app.function(**cpu_options,cpu=2,memory=8192,timeout=1800)
def check_gate(decision_sha256:str,stage:str='continuation'):
    inputs.reload();volume.reload();manifest,_,allowed=load_gate(decision_sha256,stage)
    return {'ready_for_generation':True,'allowed_sources':sorted(allowed),'config_allowlist':manifest['config_allowlist'],
        'pending_rows':manifest['pending_rows'],'diagnostic_canaries':64,'approved_for_release':False}
...
class Generator:
    ...
    def chunk(self, source_name, index, decision_sha256, stage):
        ...
        binding = {'source': source_name, 'chunk': index, 'input_sha256': spec['sha256'],
            ...
            'corrected_probe_decision_sha256': PARENT_PROBE_DECISION_SHA256,
            'config_selection': manifest['config_selection'], 'config_allowlist': manifest['config_allowlist']}
...
def finish_report(manifest, stage, allowed, state, path):
    ...
    report['corrected_probe_decision_sha256'] = PARENT_PROBE_DECISION_SHA256
    report['remaining_task_ids_sha256'] = manifest['remaining_task_ids_sha256']
    report['config_allowlist'] = manifest['config_allowlist']
    report['config_selection'] = manifest['config_selection']
    report['plan_accounting'] = manifest['plan_accounting']
    report['diagnostic_canary'] = manifest['diagnostic_canary']
