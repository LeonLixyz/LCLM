#!/usr/bin/env python3
"""
Convert safetensors checkpoint to HuggingFace format for easy loading
This script is for checkpoints that are already in merged safetensors format (model.safetensors).
"""
...
def convert_safetensors_to_hf(
    ...
    """
    Convert safetensors checkpoint to HuggingFace format
    ...
    print(f"Converting safetensors checkpoint: {checkpoint_path}")
    ...
    if not os.path.isfile(safetensors_file):
        raise ValueError(f"Safetensors file not found: {safetensors_file}")
    ...
    # Step 1: Load the safetensors state dict
    print("Step 1: Loading safetensors state dict...")
    merged_state_dict = load_safetensors(safetensors_file)
    print(f"Loaded {len(merged_state_dict)} parameters from safetensors file")
    ...
    print("Step 3: Loading weights from safetensors...")
